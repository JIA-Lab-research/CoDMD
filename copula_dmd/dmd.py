from copula_dmd.models.model_interface import InferencePipelineInterface
from copula_dmd.models import (
    get_diffusion_wrapper,
    get_text_encoder_wrapper,
    get_vae_wrapper,
    get_inference_pipeline_wrapper
)
from copula_dmd.loss import get_denoising_loss
...
class DMD(nn.Module):
    def __init__(self, args, device):
        ...
        # Get model path from config (supports both 1.3B and 14B)
        model_path = getattr(args, "model_path", None)

        self.generator = get_diffusion_wrapper(
            model_name=self.generator_model_name, model_path=model_path)()
        ...
        self.real_score = get_diffusion_wrapper(
            model_name=self.real_model_name, model_path=model_path)()
        self.real_score.set_module_grad(module_grad=args.real_score_grad)

        self.fake_score = get_diffusion_wrapper(
            model_name=self.fake_model_name, model_path=model_path)()
        self.fake_score.set_module_grad(module_grad=args.fake_score_grad)
        ...
        # Copula-aware distillation loss params
        self.use_copula_loss = getattr(args, "use_copula_loss", False)
        self.copula_loss_weight = getattr(args, "copula_loss_weight", 0.1)
        self.copula_temperature = getattr(args, "copula_temperature", 0.1)
        self.copula_batch_weight = getattr(args, "copula_batch_weight", 1.0)
        self.copula_frame_weight = getattr(args, "copula_frame_weight", 1.0)
    ...
    def generator_loss(self, image_or_video_shape, conditional_dict: dict, unconditional_dict: dict, clean_latent: torch.Tensor) -> Tuple[torch.Tensor, dict]:
        ...
        # Step 3: Copula-aware distillation loss
        # Matches relational structure (similarity matrix) between student and
        # teacher/critic predictions via KL divergence on softmax similarity matrices.
        if self.use_copula_loss:
            from copula_dmd.copula_loss import compute_copula_aware_loss
            real_score_latent = dmd_log_dict["dmdtrain_pred_real_image"]   # detached
            fake_score_latent = dmd_log_dict["dmdtrain_pred_fake_image"]   # detached
            student_latent = pred_image  # has grad
            copula_loss, copula_log_dict = compute_copula_aware_loss(
                student_latent, real_score_latent, fake_score_latent,
                temperature=self.copula_temperature,
                batch_weight=self.copula_batch_weight,
                frame_weight=self.copula_frame_weight,
            )
            dmd_loss = dmd_loss + self.copula_loss_weight * copula_loss
            dmd_log_dict.update(copula_log_dict)
            dmd_log_dict["copula_loss"] = copula_loss.detach()
    ...
    def critic_loss(self, image_or_video_shape, conditional_dict: dict, unconditional_dict: dict, clean_latent: torch.Tensor) -> Tuple[torch.Tensor, dict]:
        ...
        if self.args.denoising_loss_type == "flow":
            ...
            from copula_dmd.models.wan.wan_wrapper import WanDiffusionWrapper
